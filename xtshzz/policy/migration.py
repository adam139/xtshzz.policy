...
#    wf = getToolByName(context, 'portal_workflow')
    ...
    for bn in bns:
        ...
        try:
            ...
            IRichText(item).text = RichTextValue(
...
#    wf = getToolByName(context, 'portal_workflow')
    ...
    bns = pc(query)
    for bn in bns:
        # after modify workflow and add directly publish transition.
        if bn.review_state == "published":
            ob = bn.getObject()
            ob.setLayout("view")
    ...
    for bn in bns:
        if bn.review_state == "private":
            ...
            wf.doActionFor(
                ob,
                'publish',
                comment='old org init as published status')
