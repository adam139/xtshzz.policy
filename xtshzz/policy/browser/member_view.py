...
class MemberUrlView(grok.View):
    ...
    def render(self):
        ...
        try:
            member = catalog({'object_provides': IMember.__identifier__, "email": email})[
                0].getObject()
...
class MembraneMemberView(grok.View):
    ...
    def canRead(self):
        ...
        if id == context.email:
            return True
        elif not sm.checkPermission(ModifyPortalContent, context):
            return False
        else:
            return True
...
# is sponsor :government department
            ...
            for bn in bns:
                ob = bn.getObject()
                dview = getMultiAdapter(
                    (ob, self.request), name=u"sponsorview")
...
class ManagedOrgMore(grok.View):
    ...
    def render(self):
        ...
        favorite_view = getMultiAdapter(
            (self.context, self.request), name=u"managed_orgs_b3_view")
...
# avoid autoform functionality
    ...
    @property
    def fields(self):
        return field.Fields(IProvidePasswords).select(
