...
# title value is bytestr that encoded by utf-8
    ...
    for i in bns:
        title = i.Title
        if title in orglist:
            if i.orgnization_belondtoArea == "xiangtanshi":
                continue
            obj = i.getObject()
...
#             "orgnization_orgnizationType":"minfei"}
    ...
    for i in bns:
        ...
        ), 'id': '2014', 'object_provides': IOrgnization_annual_survey.__identifier__})
        num = len(brains)
#            import pdb
#            pdb.set_trace()
        if num == 0:
...
#                import pdb
#                pdb.set_trace()
            item.report = namedfile.NamedBlobFile(
                u'已提交纸质年检报告', filename=u"2014report.doc")
